...
export class HttpProjectionsManager implements ProjectionsManager {
  ...
  getAll(): Observable<ProjectionDetails[]> {
    return this.httpService.get<ProjectionDetails[]>("/projections/any")
        .pipe(
  ...
  get(name: string): Observable<ProjectionDetails | undefined> {
    return this.httpService.get<ProjectionDetails>(`/projection/${name}`)
        .pipe(
            map(response => response.data),
            catchError(err => {
  ...
  getQuery(name: string): Observable<string | undefined> {
    return this.httpService.get<string>(`/projection/${name}/query`)
        .pipe(
            map(response => response.data),
            catchError(err => {
              const projectionNotExists = err.response.status === 404;
  ...
  updateQuery(name: string, query: string): Observable<boolean> {
    return this.httpService.put<string>(`/projection/${name}/query?type=JS`, query)
        .pipe(
            map(response => {
  ...
  enable(name: string): Observable<void> {
    return this.httpService.post<void>(`/projection/${name}/command/enable`)
        .pipe(
            map(response => response.data)
        )
  }
  ...
  createContinuous(name: string, query: string): Observable<boolean> {
    return this.httpService.post<void>(`/projections/continuous?name=${name}&type=JS&emit=true`, query)
        .pipe(
  ...
  getState<T>(name: string): Observable<T | undefined> {
    return this.httpService.get<T>(`/projection/${name}/state`)
        .pipe(
